...
impl Plugin for StructurePlugin {
    fn build(&self, app: &mut App) {
        ...
        app
            .add_event::<LocalLociStratumSpawned>()
...
impl StructurePlugin {
    ...
    fn observer_configures_new_granite_root(
        ...
        let ordonnance_stratum = commands.spawn(OrdonnanceStratum).trigger(LocalOrdonnanceStratumSpawned).id();

        commands.entity(granite_node_entity)
    ...
    fn observer_configures_new_local_ordonnance_stratum(
        ...
        let Ok((ordonnance_stratum_entity, mut ordonnance_stratum_node)) = ordonnance_stratum_query.get_mut(trigger.entity()) else {
            error!("[STRUCTURE] Could not get newly spawned OrdonnanceStratum for configuration!");
            return;
        }; 

        let loci_stratum = commands.spawn(LociStratum).trigger(LocalLociStratumSpawned).id();

        // Stratum nodes are layered over the GraniteRoot node, for wholesale network replication.
        commands.entity(ordonnance_stratum_entity)
    ...
    fn observer_configures_new_local_loci_stratum(
        ...
        mut commands: Commands,
        mut loci_stratum_query: Query<(Entity, &mut Node), (With<LociStratum>, Without<Replicated>)>
    ) {
        let Ok((loci_stratum_entity, mut loci_stratum_node)) = loci_stratum_query.get_mut(trigger.entity()) else {
